		...
		<c:if test="${not empty param.submit }">
			...
			<c:forEach items="${course.rows}" var="course" varStatus="rowCounter">
				...
				<sql:query var="offering" dataSource="jdbc/covid">
	            	select id,seqnum,delivery_date,to_char(to_timestamp(start_time::text),'HH12:MM PM TZ') start_time,end_time,enrolled,enrollment_limit from n3c_training.offering_detail where id = ?::int and delivery_date > now() order by delivery_date,start_time;
	            	<sql:param>${course.id}</sql:param>
	            </sql:query>
